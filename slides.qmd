---
title: "Enhancing Agentic RAG:"
subtitle: "Optimizing Retrieval for an Autonomous Academic Advising Agent"
...
-   **Modular RAG:** Transitions the pipeline from a linear flow to an
    ...
    relevancy and iterates, and adaptive retrieval, which intelligently
    decides if the RAG process is even necessary for a given prompt
    [@gao2024rag].

-   **Agentic RAG:** Agentic AI systems add autonomous planning and
    decision-making on top of the RAG architecture
    [@AgenticAI2025Jagga].
...
-   The system utilizes a command-line AI agent built on the `LangGraph`
    framework, powered by OpenAI’s `gpt-5.2`, enabling state
    persistence, multi-turn memory, and recursive loops.
...
![Agentic RAG Process Flow Diagram](Images/High_Level_Agentic_RAG.png)

## Knowledge Base Data Sources {.smaller}
...
-   Employed a hierarchical-recursive chunking strategy, essentially
    preserving the page's structure and keeping relevant information
    together.
...
![Knowledge Base Embedding Pipeline](Images/KB_Ingestion_High_Level.png)
...
-   Baseline Chunking Strategy (H1/H4 Structural Splitting): 5386 chunks
...
: Knowledge Base Chunking Strategy - Chunk Size Breakdown.
{#tbl-chunk-size-breakdown}

## Knowledge Base Optimization #1: Chunking Strategy {.smaller}
...
## Knowledge Base Optimization #1: Chunk Reduction

```{r}
library(ggplot2)

iterations <- c("Baseline", "H1/H2\nChunking")

precision <- c(0.74, 0.74)
recall    <- c(0.78, 0.85)
chunks    <- c(5386, 4091)

df_lines <- data.frame(
  iteration = rep(factor(iterations, levels = iterations), 2),
  value     = c(precision, recall),
  metric    = rep(c("Context Precision", "Context Recall"), each = 2)
)

df_bars <- data.frame(
  iteration = factor(iterations, levels = iterations),
  chunks    = chunks
)
...
rescale_to_score <- function(x) {
  score_min + (x - chunk_min) / (chunk_max - chunk_min) * (score_max - score_min)
}

rescale_to_chunks <- function(y) {
  chunk_min + (y - score_min) / (score_max - score_min) * (chunk_max - chunk_min)
}

df_bars$chunks_scaled <- rescale_to_score(df_bars$chunks)

ggplot() +
  geom_col(
    data      = df_bars,
    aes(x = iteration, y = chunks_scaled),
    fill      = "#E24B4A",
    alpha     = 0.40,
    colour    = "#E24B4A",
    linewidth = 0.5,
  ...
  geom_line(
    data      = df_lines,
    aes(x = iteration, y = value, colour = metric, linetype = metric, group = metric),
    linewidth = 1.0
  ) +
  geom_point(
    data  = df_lines,
    aes(x = iteration, y = value, colour = metric, shape = metric),
    size  = 3.5
  ) +
  scale_colour_manual(
    values = c("Context Precision" = "#378ADD", "Context Recall" = "#1D9E75")
  ) +
  scale_linetype_manual(
    values = c("Context Precision" = "solid", "Context Recall" = "dashed")
  ) +
  scale_shape_manual(
    values = c("Context Precision" = 16, "Context Recall" = 17)
  ) +
  scale_y_continuous(
    name   = "Avg. Context Precision / Recall",
    breaks = seq(score_min, score_max, by = 0.10),
    labels = scales::number_format(accuracy = 0.01),
    sec.axis = sec_axis(
      transform = rescale_to_chunks,
      name      = "Total Chunks",
  ...
  theme(
    ...
    axis.title.y.right  = element_text(margin = margin(l = 10), colour = "#E24B4A"),
    axis.text.y.right   = element_text(colour = "#E24B4A")
  )

```

## Knowledge Base Optimization #2: Intelligent Chunk Filtering {.smaller}
...
## Knowledge Base Optimization #2: Results
...
precision <- c(0.74, 0.74, 0.77)
recall    <- c(0.78, 0.85, 0.89)
chunks    <- c(5386, 4091, 3764)

df_lines <- data.frame(
  iteration = rep(factor(iterations, levels = iterations), 2),
  value     = c(precision, recall),
  metric    = rep(c("Context Precision", "Context Recall"), each = 3)
)
...
## Results Summary: Development Dataset {.smaller}
...
-   The consistently low standard deviation across all metrics and
    ...
    evaluation is stable and repeatable.
...
-   The low standard deviation observed across five independent runs (≤
    0.01) is consistent with the stability observed during development
    dataset evaluation.
